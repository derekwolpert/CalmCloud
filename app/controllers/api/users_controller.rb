class Api::UsersController < ApplicationController
    ...
    def show
        @user = User.with_attached_profile_pic.includes(tracks: { audio_track_attachment: :blob, track_artwork_attachment: :blob} ).includes(:favorites).includes(:subscriptions).includes(:subscribers).includes(favorite_tracks: { audio_track_attachment: :blob, track_artwork_attachment: :blob}).find_by(username: params[:username])
